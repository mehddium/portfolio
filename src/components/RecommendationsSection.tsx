        ...
        <div className="lg:col-span-8 space-y-12 divide-y divide-[#232635]">
          {portfolioData.recommendations.map((rec) => (
            <figure key={rec.id} className="pt-10 first:pt-0 space-y-4">
              <div className="inline-flex items-center gap-1.5 text-xs text-[#8e94a5] font-medium">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                <span>{lang === "fr" ? "Avis vérifié d'encadrant" : "Verified supervisor review"}</span>
              </div>

              <blockquote className="text-base sm:text-lg text-[#f0f2f5] font-light leading-relaxed italic border-l-2 border-blue-500/50 pl-5">
                &ldquo;{rec.text[lang]}&rdquo;
              </blockquote>

              <figcaption className="pl-5 space-y-1 text-xs">
                <div className="font-semibold text-[#f0f2f5] tracking-tight text-sm">
                  {rec.author}
                </div>
                <div className="text-blue-400 font-medium">
                  {rec.role[lang]}
                </div>
                <div className="text-[#8e94a5]">
                  {rec.institution}
                </div>
              </figcaption>
            </figure>
